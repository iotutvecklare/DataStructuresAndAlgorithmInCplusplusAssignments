...
int main(){
    ...
    int antalDell;
    antalDell = std::count_if(std::begin(computers), std::end(computers), [](Computer c) {
        return c.getManufacturer() == "Dell";
    });

    std::cout << "Number of Dell computers: " << antalDell << std::endl;
    ...
    std::cout << "Top 10 cheapest computers:" << std::endl;
    for (int i = 0; i < 10; ++i) {
        std::cout << "Price: " << computers[i].getPrice() << std::endl;
    }


    //3. Finns det nån som skapar kostar mer än 41500 kr?
    bool expensiveComputer = std::any_of(computers.begin(), computers.end(), [](const Computer& c) {
        return c.getPrice() > 41500;
    });
    std::cout << "Is there any computer that costs more than 41500 kr? " << (expensiveComputer ? "Yes" : "No") << std::endl;
